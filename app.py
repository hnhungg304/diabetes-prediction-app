...
# Dựa vào thông báo lỗi và thông tin bạn đã cung cấp, mô hình được huấn luyện với các đặc trưng sau:
# Age, Sex, BMI, Waist_Circumference, Fasting_Blood_Glucose, HbA1c, Family_History_of_Diabetes, Previous_Gestational_Diabetes
# Các widget được sắp xếp theo đúng thứ tự này để tiện cho người dùng nhập liệu và đảm bảo khớp với mô hình.

age = st.slider("Tuổi", 1, 110, 30)

# Chuyển đổi sang st.radio cho Giới tính
sex_option = st.radio("Giới tính", ["Nam", "Nữ"])
# Chuyển đổi giới tính sang dạng số (ví dụ: Nam=0, Nữ=1 hoặc ngược lại, tùy thuộc vào cách bạn mã hóa khi huấn luyện)
# Giả sử: Nam = 0, Nữ = 1
...
fasting_blood_glucose = st.slider("Đường huyết lúc đói (mmg/dL)", 0, 300, 100)
hba1c = st.slider("Chỉ số HbA1c (%)", 0.0, 15.0, 5.7, format="%.1f")

# Chuyển đổi sang st.radio cho Tiền sử gia đình
family_history_option = st.radio("Có tiền sử gia đình mắc bệnh tiểu đường không?", ["Không", "Có"])
# Chuyển đổi lựa chọn sang số (Không=0, Có=1)
family_history_encoded = 1 if family_history_option == "Có" else 0

# Chuyển đổi sang st.radio cho Tiểu đường thai kỳ trước đây
...
# Sử dụng thứ tự chính xác mà bạn đã cung cấp:
# Age, Sex, BMI, Waist_Circumference, Fasting_Blood_Glucose, HbA1c, Family_History_of_Diabetes, Previous_Gestational_Diabetes
